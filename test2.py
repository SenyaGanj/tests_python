...
def whoisFunc(name):
    ...
    for elem in output:
        # т.к. формат вывода информации у whois для разных доменов может быть разный, попадатся домены,
        # в которы перед выводом информации о домене, находится не нужная информация помеченная %
        # нужно исключить эту информацию поиском %
